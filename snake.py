...
class SnakeGame:
    def __init__(self):
        ...
        self.score_prev_label = tk.Label(self.window, text="Scores précédents:")
        self.score_prev_label.pack()
    ...
    def retrieve_scores_from_db(self):
        ...
        for row in cursor:
            player_name = row[0]
            score = row[1]
            score_entry = "{} {}: {}\n".format(counter, player_name, score)
    ...
    def game_over(self):
        ...
        self.canvas.create_text(self.canvas_width / 2, self.canvas_height / 2 + 30, text=score_text, fill="white", font=("Arial", 16))
        self.canvas.create_text(self.canvas_width / 2, self.canvas_height / 2 + 60, text=time_text, fill="white", font=("Arial", 16))
    ...
    def update_score_text(self):
        ...
        # labels = "Rang\tNom\tScore\n"
        # self.score_text.insert(tk.END, labels)

        for score in self.scores:
            # joueur, score = score
            # score_entry = "{} - {}\n".format(joueur, score)
            # self.score_text.insert(tk.END, score_entry)
            score_entry = "{}\n".format(score)  # Modifier cette ligne pour utiliser directement score_entry
            self.score_text.insert(tk.END, score_entry)
